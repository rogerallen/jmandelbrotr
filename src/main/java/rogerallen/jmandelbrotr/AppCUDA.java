...
public class AppCUDA {
	...
	private static CUfunction mandelbrot_float_kernel, mandelbrot_double_kernel;
	...
	public static void init() {
        ...
        if ((err = JCuda.cudaGraphicsGLRegisterBuffer(cuda_pbo_handle, AppGL.shared_buf_id, cudaGraphicsRegisterFlags.cudaGraphicsRegisterFlagsNone)) != cudaError.cudaSuccess) {
        ...
        if ((err = JCudaDriver.cuModuleGetFunction(mandelbrot_float_kernel, module, cur_function)) != cudaError.cudaSuccess) {
        ...
        if ((err = JCudaDriver.cuModuleGetFunction(mandelbrot_double_kernel, module, cur_function)) != cudaError.cudaSuccess) {
			System.err.println("ERROR: ("+err+") failed to get function "+cur_function);
		} 
	...
	public static void render() {
	    ...
	    mandelbrot(devPtr, 
	    		AppGL.shared_tex_width, AppGL.shared_tex_height, 
	    		centerX, centerY, zoom, iterMult, doublePrecision
	    		);
	...
	private static CUdeviceptr mapResouce(cudaGraphicsResource cudaResource) {
		...
		int err;
		if ((err = JCudaDriver.cuGraphicsMapResources(1, new CUgraphicsResource[]{cuResource}, null)) != cudaError.cudaSuccess) {
	        System.err.println("ERROR: ("+err+") failed to map resource");
	    }
	    if ((err = JCudaDriver.cuGraphicsResourceGetMappedPointer(basePointer, new long[1], cuResource)) != cudaError.cudaSuccess) {
	...
	private static void unmapResouce(cudaGraphicsResource cudaResource) {
		CUgraphicsResource cuResource = new CUgraphicsResource(cudaResource);
		int err;
		if ((err = JCudaDriver.cuGraphicsUnmapResources(1, new CUgraphicsResource[]{cuResource}, null)) != cudaError.cudaSuccess) {
	        System.err.println("ERROR: ("+err+") failed to unmap resource");
	    };
	}
	
	private static void mandelbrot(CUdeviceptr devPtr, int w, int h, double cx, double cy, double zoom, int iter, boolean doublePrec ) {
	    int blockSize = 16; // 256 threads per block
	    int err;
	    if(doublePrec) {
		    Pointer doubleParams = Pointer.to(
		            Pointer.to(devPtr),
	    	...
	    	if((err = JCudaDriver.cuLaunchKernel(mandelbrot_double_kernel,
	    			w / blockSize, h / blockSize, 1,  // grids
		    ...
		    Pointer floatParams = Pointer.to(
		            Pointer.to(devPtr),
		            Pointer.to(new int[]{w}),
		            Pointer.to(new int[]{h}),
		    ...
		    if((err = JCudaDriver.cuLaunchKernel(mandelbrot_float_kernel,
		    		w / blockSize, h / blockSize, 1,
